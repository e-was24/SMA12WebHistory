...
import { Camera, Trash2, X, Sliders, ChevronRight, User, GraduationCap, Heart, Zap } from 'lucide-react'
...
function App() {
  ...
  const handlePhotoSubmit = async (e) => {
    ...
    try {
      ...
      const { data: publicUrlData } = supabase.storage.from('class-photos').getPublicUrl(fileName)
  ...
  return (
    <div className="app-container">
      <nav>
        <div className="container nav-content">
          ...
          <div className="nav-desktop">
            <button className="nav-link" onClick={() => setView('gallery')}>Gallery</button>
            <button className="nav-link" onClick={() => isAdmin ? setView('admin') : setIsLoginModalOpen(true)}>Admin</button>
            <button className="nav-link logout" onClick={handleLogout}>Selesai</button>
          </div>
      ...
      <AnimatePresence>{isFlashing && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flash-overlay" />}</AnimatePresence>
        ...
        {view === 'admin' ? (
             ...
             <div className="admin-grid">
               <div className="glass glass-card">
                 <div className="admin-tabs">
                    <button className={`tab-btn ${adminTab === 'photos' ? 'active' : ''}`} onClick={() => setAdminTab('photos')}>Photos</button>
                    <button className={`tab-btn ${adminTab === 'students' ? 'active' : ''}`} onClick={() => setAdminTab('students')}>Presensi</button>
                 </div>
                 {adminTab === 'photos' ? (
                   <form onSubmit={handlePhotoSubmit} className="admin-form">
                     <div className="form-group">
                       <label>Kategori</label>
                       <select className="form-input" value={newPhoto.class} onChange={e => setNewPhoto({...newPhoto, class: e.target.value})}>
                     ...
                     <div className="form-group">
                       <label>Caption</label>
                       <input type="text" className="form-input" value={newPhoto.caption} onChange={e => setNewPhoto({...newPhoto, caption: e.target.value})} placeholder="Keterangan..." />
                     </div>
                     <div className="form-group">
                       <label>Foto</label>
                   ...
                   <form onSubmit={handleStudentSubmit} className="admin-form">
                     <div className="form-group">
                       ...
                       <input type="text" className="form-input" value={newStudent.name} onChange={e => setNewStudent({...newStudent, name: e.target.value})} placeholder="Nama Lengkap" required />
                     </div>
                     <div className="form-row">
                        <div className="form-group">
                          <label>No. Absen</label>
                          <input type="number" className="form-input" value={newStudent.attendance_no} onChange={e => setNewStudent({...newStudent, attendance_no: e.target.value})} required />
                        </div>
                        <div className="form-group">
                          <label>Style</label>
                          <select className="form-input" value={newStudent.gender} onChange={e => setNewStudent({...newStudent, gender: e.target.value})}>
                     ...
                     <div className="form-group checkbox-group">
                        <input type="checkbox" id="isTeacher" checked={newStudent.is_teacher} onChange={e => setNewStudent({...newStudent, is_teacher: e.target.checked})} />
                     ...
                     <div className="form-group">
                       ...
                       <input type="file" className="form-input" onChange={e => {
                         ...
                         if (file) {
                           const reader = new FileReader()
                           reader.onloadend = () => setNewStudent({...newStudent, photo_url: reader.result})
                           reader.readAsDataURL(file)
                         }
                       }} />
                     </div>
                      ...
                      students.map(s => (
                        <div key={s.id} className="admin-item glass">
                          <div className="student-avatar-mini">
                            {s.photo_url ? <img src={s.photo_url} alt="" /> : (s.is_teacher ? '🎓' : s.attendance_no)}
                          </div>
                          <div className="item-info">
                            <p>{s.name}</p>
          ...
          <div className="fade-in">
             <header className="gallery-header">
                <motion.h1 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>Our Precious Memories</motion.h1>
                <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="text-muted">
             ...
             {loading ? <div className="loading-state">Syncing Cloud...</div> : (
               filter === 'Presensi' ? <PresensiSection students={students} /> : (
                 <div className="gallery-grid">
                   {photos.filter(p => filter === 'All' || p.class === filter).map(photo => (
    ...
    <div className="presensi-section">
      ...
      <div className="pupils-section">
        ...
        <div className="student-grid">
          {pupils.map(student => <StudentCard key={student.id} student={student} />)}
          {Array.from({ length: Math.max(0, 35 - pupils.length) }).map((_, i) => (
...
function PhotoCard({ photo }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="photo-card glass">
